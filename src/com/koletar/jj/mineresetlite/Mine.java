...
public class Mine implements ConfigurationSerializable {
    ...
    public Mine(Map<String, Object> me) {
        ...
        try {
            ...
            for (Map.Entry<String, Double> entry : sComposition.entrySet()) {
                composition.put(new SerializableBlock(entry.getKey()), MineResetLite.round(entry.getValue(), 4));
            }
        } catch (Throwable t) {
    ...
    public double getCompositionTotal() {
        return MineResetLite.round(composition.values().stream()
    ...
    private boolean isCancelled(BukkitTask task) {
        ...
        try {
            ...
            try {
                long period = (long) getFieldValue(task, "period");
                return (period == -2L);
    ...
    private static @Nullable Object getFieldValue(@NotNull Object obj, @NotNull String fieldName) {
        try {
    ...
    protected static ArrayList<CompositionEntry> mapComposition(Map<SerializableBlock, Double> compositionIn) {
        ...
        if (max < 1) {
            composition.put(new SerializableBlock("AIR"), MineResetLite.round(1 - max, 4));
            max = 1;
        }
        double i = 0;
        for (Map.Entry<SerializableBlock, Double> entry : composition.entrySet()) {
